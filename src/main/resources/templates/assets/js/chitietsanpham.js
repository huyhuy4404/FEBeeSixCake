...
app.controller("DanhGiaSanPhamController", function ($scope, $http) {
  ...
  $scope.filterReviews = function (rating) {
    ...
    if (rating === "all") {
      ...
      $scope.filteredReviews = $scope.reviews.filter(
...
app.controller("loadLoaiSanPham", function ($scope, $http) {
  ...
  $scope.category = {
    categoryname: "", // Khởi tạo giá trị rỗng
  };

  $scope.setCategoryName = function (categoryName) {
    // Gán giá trị product.category.categoryname vào category.categoryname
    $scope.category.categoryname = categoryName;

    // Gọi hàm filterProducts nếu muốn lọc sản phẩm và chuyển trang
    $scope.filterProducts(categoryName);
  };
