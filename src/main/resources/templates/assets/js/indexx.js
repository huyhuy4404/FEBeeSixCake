...
//               return product.categoryname === categoryName; // Lọc theo categoryname
//           });
...
app.controller("discountsController", function ($scope, $http) {
  ...
  $scope.DiscountedProducts = [];
  $scope.NewProducts = [];
  $scope.ActiveProducts = []; // Có thể bỏ nếu không sử dụng
  $scope.FilteredProducts = []; // Khởi tạo mảng để chứa sản phẩm đã lọc
  $scope.filcategoryname = [];
  $scope.selectedCategory = '';
  ...
  $scope.getProducts = function () {
      ...
      $http.get('http://localhost:8080/beesixcake/api/product')
          ...
          .then(function (response) {
              ...
              // Bỏ qua việc lưu sản phẩm có isactive là true
              // Chỉ cần lưu tất cả sản phẩm
              console.log('All Products:', $scope.Products);
          })
          .catch(function (error) {
              console.error("Error fetching data:", error);
