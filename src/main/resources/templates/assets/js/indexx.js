...
app.controller("FavoriteController", function ($scope, $http) {
  ...
  // Lấy tất cả sản phẩm yêu thích từ API
  $scope.getFavoriteProducts = function () {
    $http
      ...
      .then(function (response) {
        ...
        // Đếm số lượng yêu thích cho mỗi sản phẩm
        const productCounts = {};
        ...
        // Chuyển đổi đối tượng sang mảng và sắp xếp theo số lượng yêu thích
        $scope.favoriteProducts = Object.values(productCounts)
          .sort((a, b) => b.count - a.count) // Sắp xếp theo số lượng yêu thích
          .map(item => {
  ...
  // Lấy giá thấp nhất cho mỗi sản phẩm
  $scope.getProductPrices = function () {
    $http
      .get(`${API}/productdetail`)
      .then(function (response) {
        if (!Array.isArray(response.data)) {
          console.error("Dữ liệu không phải là mảng từ API chi tiết sản phẩm!");
          return;
        }
