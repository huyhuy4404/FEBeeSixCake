...
app.controller('CartController', ['$scope', '$http', function ($scope, $http) {
  ...
  .then(function (response) {
      $scope.products = response.data.map(function (item) {
          return {
              id: item.productdetail.product.idproduct, // ID sản phẩm
              name: item.productdetail.product.productname, // Tên sản phẩm
              img: item.productdetail.product.img, // Hình ảnh sản phẩm
              categoryName: item.productdetail.product.category.categoryname, // Tên loại sản phẩm
              sz: item.productdetail.size.sizename, // Kích cỡ
              price: item.productdetail.unitprice, // Giá sản phẩm
              quantity: item.quantity, // Số lượng đặt
              description: item.productdetail.product.description // Mô tả sản phẩm
          };
      });
